...
<!-- =====================end header ==========================-->
	...
	<SCRIPT type="text/javascript" src="resources/js/mainmaintenanceavdtdsimport_svi051r_edit.js?ver=${user.versionEspedsg}"></SCRIPT>	
...
<table width="100%" class="text11" cellspacing="0" border="0" cellpadding="0">
	...
	<tr>
		<td>
		<%-- space separator --%>
			<form action="mainmaintenanceavdtdsimport_svi051r_edit.do?id=${model.id}" name="formRecord" id="formRecord" method="POST" >
				...
				<td width="100%">
						...
						<table border="0" align="left">
				    	    ...
				    	    <tr>
				    	    	<c:choose>
					    	    	<c:when test="${not empty model.updateId}">
					    	    		<input type="hidden" name="svia_syav" id="svea_syav" value="${model.avd}">
					    	    		<input type="hidden" name="svia_syop" id="svea_syop" value="${model.opd}">
					    	    	...
					    	    	<c:otherwise>
										...
										<select name="svia_syav" id="svia_syav" required oninvalid="this.setCustomValidity('Obligatorisk')" oninput="setCustomValidity('')" class="inputTextMediumBlueMandatoryField">
						  					...
						  					<c:forEach var="record" items="${model.avdGeneralList}" >
						 				  		<option value="${record.koaavd}"<c:if test="${ model.record.svia_syav == record.koaavd}"> selected </c:if> >${record.koaavd}</option>
											  </c:forEach>  
											</select>
										</td>
